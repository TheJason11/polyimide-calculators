...
# ================================
# DECIMAL INPUT (fixes the .08 issue)
# ================================
...
def runtime_calculator_page():
    ...
    if go_btn:
        ...
        c.metric("Daily Output (8hr)", f"{day_out:,.0f} ft/day")
    st.markdown("**Typical line speeds (shop reality)**  \nFine <0.010\": 15–25 FPM • 0.010–0.050\": 12–20 FPM • >0.050\": 8–15 FPM (rarely >25 FPM).")
...
def wire_stretch_predictor_page():
    ...
    c1,c2,c3 = st.columns(3)
    with c1:
        d = decimal_input("Wire Diameter (in)", 0.0100, "st_d", min_value=0.001, max_value=0.200)
    with c2:
        T = decimal_input("Applied Tension (lb)", 5.0, "st_T", min_value=0.0, max_value=100.0)
    with c3:
        L = decimal_input("Wire Length (ft)", 100.0, "st_L", min_value=1.0, max_value=10000.0)
    with st.expander("Temperature and Material"):
        a,b = st.columns(2)
        with a:
            opF = decimal_input("Operating Temp (°F)", 800.0, "st_op", min_value=0.0, max_value=1200.0)
        with b:
            ambF = decimal_input("Ambient Temp (°F)", 77.0, "st_amb", min_value=0.0, max_value=120.0)
    if st.button("Predict Elongation", key="stretch"):
        if None in (d, T, L, opF, ambF):
            st.error("Please complete all inputs.")
            return
        A = circle_area(d)
        stress = T / A if A>0 else 0
        elastic = (stress / E_COPPER_PSI) * L * IN_PER_FT
        thermal = thermal_strain(opF - ambF, "copper") * L * IN_PER_FT
        total = elastic + thermal
        pct = total / (L*IN_PER_FT) * 100
        a,b,c = st.columns(3)
        a.metric("Applied Stress", f"{stress:,.0f} PSI")
        b.metric("Elastic Elongation", f"{elastic:.4f} in")
        c.metric("Thermal Elongation", f"{thermal:.4f} in")
        st.metric("Total Elongation", f"{total:.4f} in")
        st.caption(f"Percent elongation: {pct:.3f}%")

# ================================
# ANNEAL TEMP ESTIMATOR
#   kNN (log-space) + Ridge-like backbone (non-negative slopes) + Monotone guard vs speed
# ================================
def anneal_temp_estimator_page():
    ...
    st.markdown(
        ...
        "Backbone with non-negative slopes. A local isotonic correction enforces: as speed increases (dwell ↓), "
        "required temperature does not decrease."
    )
    ...
    if st.button("Predict Temperature", key="anneal_predict"):
        ...
        # features: ln(diameter), -ln(dwell) (larger = harder condition → higher T)
        ...
        # Backbone with non-negative slopes (manual ridge-like), on raw X
        lam = 10.0
        ...
        # Local monotonic guard along speed at near-constant diameter
        ...
        def isotonic_predict(ln_d_q, ln_dw_q, band=0.06, min_n=12):
            cur_band = band
            for _ in range(5):  # widen progressively if needed
        ...
        if T_iso is None:
            ...
            x_local_raw = X[I, 1]   # -ln(dwell) in raw space
            y_local = neigh_temps
        ...
        # global blend with backbone (stronger local weight to boost speed sensitivity)
        ...
        if T_iso is not None:
            st.caption("Monotonic guard active (isotonic vs speed).")
        st.caption(
        ...
        with st.expander("Closest Historical Runs"):
            ...
            # NOTE: no background_gradient here to avoid matplotlib dependency
            st.dataframe(
...
- Decimal typing fix (accepts .08 / 0.08 cleanly)
- Anneal model: monotone vs speed, non-negative backbone slopes
- Tighter diameter band for local fit (more speed sensitivity)
- Higher weight on local monotone fit
- Removed pandas background gradient to avoid ImportError
    """)
